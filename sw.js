/* ═══════════════════════════════════════════════════════════════
   건축법 상주감리 대가 산출기 — Service Worker
   건축법 제25조 · 시행령 제19조 | ARCHITECT KIM MANMIN · Ver-1.0
   전략: Cache-First (로컬 자산) + Network-First (외부 CDN)
   ═══════════════════════════════════════════════════════════════ */

const CACHE_NAME   = 'arch-supervision-v1.0';
const CDN_CACHE    = 'arch-supervision-cdn-v1.0';
...
self.addEventListener('push', (event) => {
  const data = event.data?.json() ?? { title: '상주감리 산출기', body: '업데이트가 있습니다.' };
  event.waitUntil(
